...
- **App Store 订阅价格**：通过 App Store Price 的公开套餐页采集同一订阅在各地区的原币种价格；价格变化时生成事件。
...
`rss` 使用 `feedUrl`；`app-store` 使用 `appId` 和可选的 `country`；`app-store-price` 使用 `appId`、`subscriptionId`、可选的 `planName` 和 `locale`；`google-play` 使用 `packageId`、可选的 `language`/`country`。订阅价格使用地区、币种和原始价格的完整快照判断变更，不会只因汇率折算变化产生事件。

仓库已包含 ChatGPT Plus 的订阅价格案例：`chatgpt-plus-price`。它从 [App Store Price](https://appstoreprice.org/zh/apps/6448311069) 采集 33 个地区的月度套餐价格；首次同步会记录当前快照，后续地区原币种价格发生变化时才会产生新事件。
