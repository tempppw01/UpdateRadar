...
- **App Store 内购价格**：通过 Apple 官方 App Store 页面采集订阅或其他内购套餐在指定商店地区的公开价格；价格变化时生成事件。
...
`rss` 使用 `feedUrl`；`app-store` 使用 `appId` 和可选的 `country`；`app-store-price` 使用 `appId`、`subscriptionId`、可选的 `planName`、`country` 与 `storefrontId`；`google-play` 使用 `packageId`、可选的 `language`/`country`。内购价格使用 Apple 官方页面中套餐的原始格式化价格判断变更。

仓库已包含 ChatGPT Plus 的内购价格案例：`chatgpt-plus-price`。它通过 Apple 官方美国 App Store 页面监控套餐 `oai_chatgpt_plus_1999_1m`；首次同步会记录当前价格，之后价格变动才会产生新事件。预设支持美国、中国大陆、日本、英国、加拿大、澳大利亚、德国、法国、印度、韩国、中国香港、中国台湾、新加坡、菲律宾和巴西；其他地区可在数据源中填写 Apple Storefront ID。
